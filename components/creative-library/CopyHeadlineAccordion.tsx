...
interface CopyHeadlineAccordionProps {
  filters: CopyHeadlineFilters
  onChange: (filters: Partial<CopyHeadlineFilters>) => void
  isOpen?: boolean
  onToggle?: () => void
  dynamicOptions?: {
    copyAngle?: string[]
          ...
          <div className="space-y-1 max-h-60 overflow-y-auto">
            {options.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-6 text-center">
                ...
                <span className="text-white text-sm">No copy options found</span>
              </div>
            ) : (
              options.map((option) => (
                <Button
...
export const CopyHeadlineAccordion: React.FC<CopyHeadlineAccordionProps> = ({
  filters,
  onChange,
  isOpen = false,
  onToggle,
  ...
  const copyAngleOptions = dynamicOptions?.copyAngle || [
    'Urgency', 'Authority', 'Social Proof', 'Scarcity', 'Value-Focused', 'Quality-Focused', 'Convenience', 'Problem-Solving', 'Emotional Appeal'
  ]

  const copyToneOptions = dynamicOptions?.copyTone || [
    'Formal', 'Casual', 'Persuasive', 'Professional', 'Friendly', 'Urgent', 'Playful', 'Sophisticated', 'Conversational', 'Authoritative'
  ]

  return (
    <div className="w-full" data-copy-headline-accordion="true">
      {/* Accordion Header */}
      <button
        onClick={onToggle}
      ...
      >
        <span className="text-white font-medium text-sm">Copy & Headline</span>
        <ChevronDown 
          className={cn(
